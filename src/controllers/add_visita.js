...
export const addVisitaForm = async (req, res) => {
    try {
        ...
        connection.query('INSERT INTO jefe_hogar (rut,nombre,edad,fecha_nac,direccion,nacionalidad,estado_civil,celular,apellidos,correo) VALUES (?,?,?,?,?,?,?,?,?,?)',
        [jh_rut,jh_name,jh_age,jh_birthdate,jh_address,jh_nat,jh_marital_status,jh_phone,jh_last_name,jh_email],(error, results) => {
                if (error) {
                    console.log(error);
                } else {
                    res.render('register', {
